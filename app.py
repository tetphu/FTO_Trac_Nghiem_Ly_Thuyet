...
def inject_css():
    st.markdown("""
        ...
        /* Đồng hồ đếm ngược */
        .timer-box {
            font-size: 40px; font-weight: bold; color: #d9534f;
            text-align: center; margin: 10px 0;
            border: 2px solid #d9534f; border-radius: 10px;
            padding: 5px; width: 100px; margin-left: auto; margin-right: auto;
        ...
        # 4. THI CỬ
            ...
            if not st.session_state['bat_dau']:
                ...
                if idx >= len(qs):
                    ...
                    if st.button("KẾT THÚC VÀ NỘP BÀI"):
                ...
                if not st.session_state['da_nop_cau']:
                    ...
                    # HIỂN THỊ ĐỒNG HỒ SỐ (KHÔNG CÓ THANH CHẠY)
                    ...
                    # Đáp án
                    ans = st.radio("Lựa chọn của bạn:", [f"A. {q[1]}", f"B. {q[2]}", f"C. {q[3]}", f"D. {q[4]}"])
                ...
                # --- PHẦN 2: ĐÃ TRẢ LỜI (HIỆN LẠI CÂU HỎI + GIẢI THÍCH) ---
                else:
                    st.markdown(f"**Câu {idx+1}:**")
                    # Hiển thị lại câu hỏi để đối chiếu
                    st.markdown(f"<div class='question-box'>{q[0]}</div>", unsafe_allow_html=True)
                    ...
                    # So sánh kết quả
                    if res == true_ans:
                        st.success(f"✅ CHÍNH XÁC! Bạn chọn: {res}")
                    else:
                        st.error(f"❌ SAI RỒI! Bạn chọn: {res if res else 'Không chọn'}")
                        st.warning(f"👉 Đáp án đúng là: {true_ans}")

                    # Hiện giải thích
                    explanation = str(q[6]).strip()
                    if explanation:
                        st.markdown(f"<div class='explanation-box'>💡 <b>Giải thích chi tiết:</b><br>{explanation}</div>", unsafe_allow_html=True)
                    else:
                        st.info("💡 (Không có giải thích chi tiết cho câu này)")
