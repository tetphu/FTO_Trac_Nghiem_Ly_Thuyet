...
# --- KẾT NỐI GOOGLE SHEET (BẮT LỖI CHẶT CHẼ) ---
def ket_noi_csdl():
    try:
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        
        # Kiểm tra Secrets hoặc File JSON
        if "gcp_service_account" in st.secrets:
            creds = ServiceAccountCredentials.from_json_keyfile_dict(st.secrets["gcp_service_account"], scope)
        else:
            # Nếu chạy trên máy cá nhân cần file credentials.json
            creds = ServiceAccountCredentials.from_json_keyfile_name("credentials.json", scope)
            
        client = gspread.authorize(creds)
        return client.open("HeThongTracNghiem")
    except Exception as e:
        # In lỗi ra màn hình để không bị trắng xóa
        st.error(f"⚠️ LỖI KẾT NỐI GOOGLE SHEET: {str(e)}")
        st.info("Hãy kiểm tra lại: 1. Tên file Google Sheet có đúng là 'HeThongTracNghiem' không? 2. Đã Share quyền cho Email Service Account chưa?")
        return None
...
def main():
    ...
    # --- CSS AN TOÀN (Đã kiểm tra kỹ) ---
    st.markdown("""
        <style>
        /* Căn chỉnh lề trang web */
        .block-container { 
            padding-top: 1rem; 
            padding-bottom: 5rem; 
            max-width: 900px; 
        ...
        /* KHUNG VIỀN CHO FORM (Chỉ khung này có viền) */
        .form-box {
    ...
    # --- KHỞI TẠO SESSION STATE (Rất quan trọng để tránh lỗi trắng màn hình) ---
    ...
    # Kết nối Database
    db = ket_noi_csdl()
    if not db: st.stop() # Dừng lại nếu không kết nối được

    # --- PHẦN HEADER (Logo Trái - Chữ Phải) ---
    col1, col2 = st.columns([1, 2.5])
    with col1:
        st.image("https://github.com/tetphu/FTO_Trac_Nghiem_Ly_Thuyet/blob/main/GCPD%20(2).png?raw=true", width=230)
    with col2:
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("TRUY CẬP HỆ THỐNG"):
                ...
                if vt == "DA_KHOA": st.error("⛔ HỒ SƠ ĐÃ KHÓA")
                    ...
                    # Reset toàn bộ trạng thái khi đăng nhập mới
    ...
    # ==========================================
        ...
        if not st.session_state['bat_dau']:
            ...
            st.markdown("""
                ...
                <p style="font-size:18px; font-weight:bold;">Chúc Sĩ Quan thi tốt</p>
            """, unsafe_allow_html=True)
            st.write("")
